  ...
  function Question(type) {
      ...
      var insert = function(order, id, elements, $container) {
          ...
          for (var i = 0; i < order.length; i++) {
              if (order[i] === id) {
                  // Found our pos
                  while (i > 0 && (elements[order[i - 1]] === undefined || !elements[order[i - 1]].isVisible)) {
                      i--;
                  }
                  if (i === 0) {
                      // We are on top.
          ...
          buttonTruncationTimer = setTimeout(function() {
              ...
              for (var i in buttons) {
                  ...
                  if (buttons[i].isVisible) {
                      ...
                      var buttonInstanceWidth = $element.get(0).offsetWidth + parseFloat($element.css('margin-left')) + parseFloat($element.css('margin-right'));
      ...
      var removeButtonLabels = function(buttonsWidth, maxButtonsWidth) {
          ...
          for (var i = buttonOrder.length - 1; i >= 0; i--) {
              ...
              if (!buttons[buttonId].isTruncated && buttons[buttonId].isVisible) {
                  ...
                  var $tmp = $button.clone().css({
                      'position': 'absolute',
                      'white-space': 'nowrap',
                      'max-width': 'none'
                  }).addClass('truncated').html('').appendTo($button.parent());
          ...
          for (var i = 0; i < buttonOrder.length; i++) {
              ...
              if (buttons[buttonId].isTruncated && buttons[buttonId].isVisible) {
                  ...
                  var $tmp = $button.clone().css({
                      ...
                      'white-space': 'nowrap',
                      'max-width': 'none'
                  }).removeClass('truncated').html(buttons[buttonId].text).appendTo($button.parent());
      ...
      self.setImage = function(path, options) {
          ...
          if (options.disableImageZooming) {
              ...
              var determineImgWidth = function() {
                  ...
                  var diffX = 2 * ($imgWrap.get(0).getBoundingClientRect().left - sections.image.$element.get(0).getBoundingClientRect().left);
          ...
          var determineSize = function() {
              ...
              $imgWrap.addClass('h5p-question-image-scalable').attr('aria-expanded', false).attr('role', 'button').attr('tabIndex', '0').on('click', function(event) {
                  if (event.which === 1) {
      ...
      self.readFeedback = function() {
          var invalidFeedback = behaviour.disableReadSpeaker || !showFeedback || !sections.feedback || !sections.feedback.$element;
          if (invalidFeedback) {
              return;
          }
      ...
      self.addButton = function(id, text, clicked, visible, options, extras) {
          ...
          options = options || {};
          var confirmationDialog = self.addConfirmationDialogToButton(extras.confirmationDialog, clicked);
          /**
      ...
      self.attach = function($container) {
          ...
          if ($wrapper === undefined) {
              if (self.registerDomElements !== undefined && (self.registerDomElements instanceof Function || typeof self.registerDomElements === 'function')) {
...
H5P.Summary = (function($, Question, XApiEventBuilder, StopWatch) {
  function Summary(options, contentId, contentData) {
      ...
      if (contentData && contentData.previousState !== undefined && contentData.previousState.progress !== undefined && contentData.previousState.answers) {
  ...
  Summary.prototype.createQuestion = function() {
      ...
      var selectedAlt = function($el, setFocus) {
          ...
          if (that.answer[nodeId]) {
              ...
              if (elements[that.progress] !== undefined && elements[that.progress].tip !== undefined && elements[that.progress].tip.trim().length > 0) {
          ...
          if (that.userResponses[panelId].length === 1) {
              that.trigger(that.createXApiAnsweredEvent(that.summaries[panelId], that.userResponses[panelId] || [], panelId, that.timePassedInStopWatch(panelId)));
          }
      ...
      for (var i = 0; i < elements.length; i++) {
          ...
          for (var j = 0; j < element.summaries.length; j++) {
              ...
              var $node = $('' + '<li role="button" tabindex="0" data-bit="' + element.summaries[j].id + '" class="' + summaryLineClass + '">' + element.summaries[j].text + '</li>');
  ...
  Summary.prototype.createXApiAnsweredEvent = function(panel, userAnswer, panelIndex, duration) {
      ...
      var definition = XApiEventBuilder.createDefinition().name('Summary statement').description(self.options.intro).interactionType(XApiEventBuilder.interactionTypes.CHOICE).correctResponsesPattern(['0']).optional(self.getXApiChoices(panel.summary)).build();
      // create the result object
      var result = XApiEventBuilder.createResult().response(userAnswer.join('[,]')).duration(duration).score((self.correctOnFirstTry(userAnswer) ? 1 : 0), 1).build();
      return XApiEventBuilder.create().verb(XApiEventBuilder.verbs.ANSWERED).objectDefinition(definition).context(self.contentId, self.subContentId).contentId(self.contentId, panel.subContentId).result(result).build();
  };
  ...
  Summary.prototype.getXAPIData = function() {
      ...
      // creates the definition object
      var definition = XApiEventBuilder.createDefinition().interactionType(XApiEventBuilder.interactionTypes.COMPOUND).name(self.getTitle()).description(self.options.intro).build();
